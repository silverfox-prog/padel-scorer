...
import { getPlayerStats, getPlayerHistory, getHeadToHeadStats, getPartnerStats } from "../../../lib/dataStore";
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setPartners(allPartners.filter((p) => p.player_1_name === playerName || p.player_2_name === playerName));
        setOpponents(allH2h.filter((p) => p.player_1_name === playerName || p.player_2_name === playerName));
      } catch (e) {
          ...
          <>
            ...
            {tab === "partners" && (
              <div style={{ background: "rgba(244,239,230,0.08)", borderRadius: 12, overflow: "hidden" }}>
                {partners.length === 0 ? (
                  <div style={{ padding: 16, textAlign: "center", opacity: 0.7, fontSize: 13.5 }}>No partner data yet.</div>
                ) : (
                  partners.map((p, i) => (
                    <div
                      key={i}
                      style={{ padding: "12px 16px", borderBottom: i < partners.length - 1 ? "1px solid rgba(244,239,230,0.1)" : "none" }}
                    >
            ...
            {tab === "opponents" && (
              <div style={{ background: "rgba(244,239,230,0.08)", borderRadius: 12, overflow: "hidden" }}>
                {opponents.length === 0 ? (
                  <div style={{ padding: 16, textAlign: "center", opacity: 0.7, fontSize: 13.5 }}>No head-to-head data yet.</div>
                ) : (
                  opponents.map((h, i) => {
                    const theirName = h.player_1_name === playerName ? h.player_2_name : h.player_1_name;
                    const playerWins = h.player_1_name === playerName ? h.player_1_wins : h.matches_played - h.player_1_wins;
                    return (
                      <div
                        key={i}
                        style={{ padding: "12px 16px", borderBottom: i < opponents.length - 1 ? "1px solid rgba(244,239,230,0.1)" : "none" }}
                      >
            ...
            {tab === "history" && (
              <div style={{ background: "rgba(244,239,230,0.08)", borderRadius: 12, overflow: "hidden" }}>
                {history.length === 0 ? (
                  <div style={{ padding: 16, textAlign: "center", opacity: 0.7, fontSize: 13.5 }}>No games yet.</div>
                ) : (
                  history.map((round, i) => (
                    <button
                      key={round.id}
                    ...
                    >
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 12, opacity: 0.6, marginBottom: 2 }}>{round.sessions.played_on}</div>
                        <div style={{ fontSize: 13.5, fontWeight: 600, textTransform: "capitalize" }}>{round.sessions.mode}</div>
                      </div>
