...
export async function getSessionHistory(limit = 50) {
  ...
  return data;
/**
 * Delete a single round by ID. Also recalculate any dependent player stats.
 */
...
export async function getPlayerHistory(playerName) {
  ...
  if (findErr) throw findErr;
  if (!player) return { player: null, sessions: [] };
...
/**
 * Get all rounds for a specific session with player names resolved.
 */
